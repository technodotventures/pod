...
export class SearchIndex {
    ...
    /** Count indexed pages */
    count() {
        return this.db.prepare('SELECT COUNT(*) as c FROM search_index').get().c;
    }
    /**
     ...
     * fails or times out.
     */
    indexEntityFromClaims(entity, claims) {
    ...
    indexClaim(claimId, subjectName, scope, content) {
    ...
    replaceClaimIndex(claims, scope) {
    ...
    `);
        const replace = this.db.transaction(() => {
            if (scope)
                this.db.prepare('DELETE FROM claim_search_index WHERE scope = ?').run(scope);
            else
            ...
            for (const claim of claims) {
                ...
                insert.run(claim.id, claim.subject_id, claim.subject_name, claim.scope, claim.predicate, `${claim.subject_name}\n${claim.predicate}: ${content}`);
            }
        });
        replace();
    }
 ...
 */
export function syncSearchFromClaims(store, searchIndex, scope) {
    ...
    let indexed = 0;
    const indexedEntityIds = new Set();
    ...
    for (const claim of allActive) {
        if (indexedEntityIds.has(claim.subject_id))
            continue;
