...
export function generateManifest(config, stats) {
    const now = new Date().toISOString();
    const scopeList = config.scopes.map(s => `  - \`${s.id}\` (${s.visibility_default}${s.parent ? `, parent: ${s.parent}` : ''})`).join('\n');
...
export function countWikiPages(wikiDir) {
    let count = 0;
    ...
    const scan = (dir) => {
        ...
        for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
            ...
            if (entry.isDirectory()) {
                ...
                if (path.resolve(full) === rootManifest)
                    continue;
                count++;
            }
        }
