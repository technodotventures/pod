...
import { compileMarkdownLLM } from '../extraction/llm.js';
...
function stripUserMetadataForExtraction(value) {
...
export function isContextOnlyObservation(obs, contextClaimIds) {
...
export async function compile(evidenceDir, wikiDir, layer0, store, config, options = {}, searchIndex) {
    ...
    const contextClaimIds = new Set(store.getActiveClaims(options.scope)
    ...
    const entities = options.entityId
        ? [store.getEntity(options.entityId)].filter(Boolean)
    ...
    for (const entity of entities) {
        ...
        if (llmConfigured && sensitive)
            llmSynthesisSkippedSensitive++;
        let usedLLM = false;
        if (llmConfigured && !sensitive) {
            llmSynthesisAttempted++;
            try {
                const llmResult = await compileMarkdownLLM(entity.canonical_name, entity.type, claims.map(c => ({ predicate: c.predicate, object: c.object, epistemic: c.epistemic, confidence: c.confidence })), null, config);
        ...
        const page = { path: pagePath, frontmatter, oneliner, paragraph, fullPage, raw };
        pages.push(page);
 ...
 */
function entityPath(wikiDir, entity) {
    const category = categorizeEntity(entity);
    const slug = entitySlug(entity);
    return path.join(wikiDir, category, `${slug}.md`);
}
function mostConfidentEpistemic(claims) {
    const order = ['user_confirmed', 'asserted', 'observed', 'inferred', 'system_generated'];
    for (const label of order) {
        if (claims.some(c => c.epistemic === label))
            return label;
    }
    return 'inferred';
}
/** Ensure all six spec-shaped category directories exist. Idempotent. */
function ensureCategoryDirs(wikiDir) {
 ...
 */
function rebuildCategoryIndex(wikiDir, category) {
...
function buildEvidenceTimeline(claims, compiledAt) {
    const lines = [];
    lines.push('## Evidence Timeline');
    lines.push('');
    lines.push(`<!-- cached view — compiled_at: ${compiledAt} -->`);
    lines.push('');
    const sorted = [...claims].sort((a, b) => (a.extraction.extracted_at ?? '').localeCompare(b.extraction.extracted_at ?? ''));
...
function replaceEvidenceTimeline(existingBody, newTimeline) {
