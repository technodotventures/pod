...
function capabilityList(grant) {
    const ops = Object.keys(grant.capabilities).filter((op) => (grant.capabilities[op] ?? []).length > 0);
    return ops.join(', ') || '(none)';
}
function actorKind(grant) {
...
function ownerCell(grant) {
    return grant.trusted ? 'self' : 'shared';
}
function escapePipe(value) {
    return value.replace(/\|/g, '\\|');
}
export function renderRegistryMarkdown(config) {
    ...
    // Pod owner row first.
    if (config.owner_id) {
        lines.push(`| ${escapePipe(config.owner_id)} | human | self | all | - | Pod owner |`);
    }
 ...
 */
export function writeRegistryMarkdown(dataDir, config) {
...
//# sourceMappingURL=registry-md.js.map