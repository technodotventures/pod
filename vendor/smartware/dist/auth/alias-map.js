 ...
 */
export function appendAlias(dataDir, entry) {
    const final = {
 ...
 */
export function resolveActorId(map, actorId) {
 ...
 */
export const COFFEE_POD_DEFAULT_ALIASES = [
...
export function ensureDefaultAliases(dataDir) {
    const existing = loadAliasMap(dataDir);
    for (const entry of COFFEE_POD_DEFAULT_ALIASES) {
        if (existing.get(entry.legacy) === entry.spec)
            continue;
        appendAlias(dataDir, {
