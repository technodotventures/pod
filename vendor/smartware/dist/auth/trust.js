 ...
 */
export function shouldQuarantine(_actor, grant) {
    if (!grant)
        return true;
    if (grant.quarantine)
        return true;
 ...
 */
export function quarantineForGrants(grants) {
    if (grants.length === 0)
        return true;
    if (grants.some(g => g.quarantine))
        return true;
    if (grants.some(g => g.trusted))
        return false;
 ...
 */
export function canQueryLowConfidence(grant) {
    if (!grant)
        return false;
