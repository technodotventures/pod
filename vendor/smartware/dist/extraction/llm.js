...
async function requestAnthropicText(config, system, prompt, maxTokens) {
...
async function requestOpenAIText(config, system, prompt, maxTokens) {
    const headers = {
...
async function requestOpenRouterText(config, system, prompt, maxTokens) {
...
async function requestLLMText(config, options) {
...
export async function extractClaimsLLM(content, scope, validityFrom, existingEntities, config) {
    if (config.llm.provider === 'none') {
    ...
    const claims = rawClaims
        ...
        .map(c => normaliseRawClaim(c, scope, validityFrom, config.llm.model, promptHash));
...
function computePromptHash(text) {
    return createHash('sha256').update(text).digest('hex').slice(0, 16);
}
/** Generate a compiled markdown page using the active LLM provider */
export async function compileMarkdownLLM(entityName, entityType, claims, existingPage, config) {
