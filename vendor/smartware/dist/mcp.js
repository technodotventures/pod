...
function actor(id, type = 'agent', displayName = id) {
    return { type, id, display_name: displayName };
}
function requireIdentity(actorId, sessionId) {
 ...
 */
export function createSmartwareMcpServer(core, options = {}) {
    ...
    async function wrap(fn, label) {
        let timeout;
        try {
            const result = await Promise.race([
                fn(),
                new Promise((_resolve, reject) => {
                    timeout = setTimeout(() => reject(new Error(`Handler '${label}' timed out after ${timeoutMs}ms`)), timeoutMs);
                }),
            ]);
            return {
    ...
    const compile = (args) => core.compile({
    ...
    server.tool('smartware_reflect', 'Reflect accepted evidence into derived claims and projections', compilationShape, async (args) => wrap(() => compile(args), 'reflect'));
    server.tool('smartware_compile', 'Compile knowledge into markdown wiki pages', compilationShape, async (args) => wrap(() => compile(args), 'compile'));
    server.tool('smartware_read', 'Read a compiled wiki page for an entity, or browse a scope', {
        actor_id: z.string().optional(),
        session_id: z.string().optional(),
        entity_id: z.string().optional(),
    ...
    server.tool('smartware_explain', 'Trace claim or entity provenance', {
        actor_id: z.string(),
    ...
    server.tool('smartware_correct', 'Correct an existing claim', {
        actor_id: z.string(),
    ...
    server.tool('smartware_revise', 'Revise claim admission metadata and relations', {
        actor_id: z.string(),
        target: z.string(),
    ...
    server.tool('smartware_quarantine_review', 'Approve or reject quarantined evidence', {
        actor_id: z.string(),
        target_obs_id: z.string(),
    ...
    server.tool('smartware_grant', 'Grant actor capabilities', {
        actor_id: z.string(),
    ...
    server.tool('smartware_revoke', 'Revoke a grant', {
        actor_id: z.string(),
        grant_id: z.string(),
        reason: z.string().optional(),
    ...
    server.tool('smartware_session_start', 'Start a server-anchored session', {
        actor_id: z.string(),
        client_id: z.string(),
    ...
    server.tool('smartware_session_describe', 'Describe a session', {
        actor_id: z.string(),
        session_id: z.string(),
    }, async (args) => wrap(() => core.sessionDescribe(args.actor_id, args.session_id), 'session_describe'));
    server.tool('smartware_session_end', 'End an active session', {
        actor_id: z.string(),
        session_id: z.string(),
    }, async (args) => wrap(() => core.sessionEnd(args.actor_id, args.session_id), 'session_end'));
    server.tool('smartware_status', 'Get system status', { actor_id: z.string() }, async (args) => wrap(() => core.status(args.actor_id), 'status'));
    return server;
}
