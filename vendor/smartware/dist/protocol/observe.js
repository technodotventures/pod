...
import { appendOpLogEntry, OPERATION_ID_PATTERN, readAllOpLogEntries, readOperationIntent, persistOperationIntent, removeOperationIntent, runRecovery, } from '../ops_log/index.js';
import { checkAttachmentSafety } from '../layer0/attachment_safety.js';
import { SMARTWARE_VERSION } from '../version.js';
function observePayload(params) {
    return {
...
export async function handleObserve(params, evidenceDir, layer0, config, sessionStore, opsDir, commitHooks) {
    ...
    if (secretCheck.detected) {
        throw new ProtocolError('secret_detected', `Observation rejected: secret detected (${secretCheck.type}) at ${secretCheck.location}. Remove credentials before storing.`);
    }
    ...
    const committedResult = () => {
        ...
        if (entries.length === 0)
            return null;
        const exact = entries.find(entry => entry.op === 'observe'
    ...
    if (layer0.getEffectiveStatus(observationId) !== null) {
        const existing = layer0.getDB().prepare('SELECT sequence FROM observations WHERE id = ?').get(observationId);
        return {
            id: observationId,
            status: 'duplicate',
    ...
    const intent = operationId && opsDir
        ? {
            version: 1,
