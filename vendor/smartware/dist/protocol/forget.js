...
import { readLatestVersion, appendClaimVersion, readClaimHistory, } from '../layer1/jsonl.js';
...
function normaliseTarget(params) {
    if (params.target)
        return params.target;
    if (params.target_obs_id)
        return { type: 'observation', id: params.target_obs_id };
    if (params.target_claim_id)
        return { type: 'claim', id: params.target_claim_id };
    throw new ProtocolError('invalid_target', 'A FORGET target is required');
}
function forgetPayload(params, target) {
    return {
...
export async function handleForget(params, evidenceDir, layer0, store, config, commitCtx, commitHooks) {
    ...
    const committedResult = () => {
        ...
        if (entries.length === 0)
            return null;
        const exact = entries.find(entry => entry.op === 'forget'
    ...
    if (target.type === 'observation') {
        ...
        const obsRow = layer0.getDB().prepare('SELECT scope FROM observations WHERE id = ?').get(target.id);
    ...
    const impactedClaims = store.getAllClaims().filter(claim => claim.status !== 'retracted'
        && (target.type === 'claim'
            ? claim.id === target.id
            : claim.supporting_evidence.includes(target.id)
                || claim.source_event_id === target.id
                || claim.extraction_event_id === target.id));
    const plannedClaimsRetracted = existingIntent?.result.claims_retracted ?? (target.type === 'claim'
    ...
    const plannedClaimsReduced = existingIntent?.result.claims_reduced ?? (target.type === 'observation'
        ? impactedClaims.filter(claim => claim.supporting_evidence.includes(target.id)
...
export async function handleRevive(params, dataDir, store, config, commitCtx, db, commitHooks) {
    ...
    const committedResult = () => {
        ...
        if (entries.length === 0)
            return null;
        const exact = entries.find(entry => entry.op === 'revive'
            && entry.actor_id === params.actor.id
            && entry.details?.['payload_hash'] === payloadHash);
        if (!exact) {
            throw new ProtocolError('conflict', `operation_id '${params.operation_id}' was already used with a different payload`);
    ...
    const lastActive = [...history].reverse().find(v => v.state === 'active');
    if (!lastActive) {
