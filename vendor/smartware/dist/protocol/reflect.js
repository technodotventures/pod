...
import { compile, isContextOnlyObservation } from '../layer2/compiler.js';
...
import { appendClaimVersion, readLatestVersion, iterAllClaimVersions, } from '../layer1/jsonl.js';
...
import { appendOpLogEntry, OPERATION_ID_PATTERN, persistOperationIntent, readAllOpLogEntries, removeOperationIntent, } from '../ops_log/index.js';
import { isSessionCheckpointContent, renderSessionCheckpoint, validateSessionCheckpoint, } from '../session/checkpoint.js';
function extractedEpistemic(value) {
    if (value === 'observed'
        || value === 'asserted'
        || value === 'inferred'
...
function materializeSemantic(claim, subjectType, extractedAt, sensitive) {
    const tValidFrom = claim.t_valid_from ?? {
        value: claim.validity?.from ?? extractedAt,
        state: claim.validity?.from ? 'known' : 'inferred',
        ...(claim.validity?.from ? {} : { basis: 'reflection_time' }),
    };
    const tValidTo = claim.t_valid_to ?? (claim.validity?.to
        ? { value: claim.validity.to, state: 'known' }
...
export function isReflectAutoTerminalReceipt(entry) {
...
function commitReflectClaim(dataDir, record, commitCtx, hooks) {
...
export async function handleCompile(params, evidenceDir, wikiDir, layer0, store, searchIndex, config, dataDir, commitCtx, commitHooks) {
    ...
    const entityHints = new Map();
    if (dataDir) {
        reflectionStats = await reflectAutoCreateClaims(evidenceDir, dataDir, layer0, store, targetScope, config, params.use_llm === true, commitCtx, entityHints, commitHooks);
...
async function reflectAutoCreateClaims(evidenceDir, dataDir, layer0, store, scope, config, useLLM, commitCtx, entityHints, commitHooks) {
    const podActorId = `substrate:${config.instance_id.replace('smartware_', '')}`;
    let created = 0;
    let llmAttempted = 0;
    let llmFailed = 0;
    let llmSkippedSensitive = 0;
    const processedObsIds = new Set();
    if (commitCtx) {
        ...
        for (const v of iterAllClaimVersions(dataDir)) {
            for (const obsId of v.derived_from)
                processedObsIds.add(obsId);
        }
    }
    const contextClaimIds = new Set(store.getActiveClaims(scope)
        .filter((claim) => claim.status === 'active')
        .map((claim) => claim.id));
    const markComplete = (observationId, observationScope, outcome, details = {}) => {
        if (commitCtx) {
            const receipt = {
                ...details,
    ...
    for (const obs of readAll(evidenceDir)) {
        if (obs.status !== 'accepted')
            continue;
        if (obs.type === 'claim_extracted' || obs.type === 'correction' || obs.type === 'tombstone')
            continue;
        if (scope && obs.scope !== scope && !obs.scope.endsWith('/' + scope) && !obs.scope.startsWith(scope + '/'))
            continue;
        if (processedObsIds.has(obs.id))
            continue;
        ...
        if (typeof obs.content.body === 'string') {
            ...
            const inner = obs.content.body.body;
        ...
        if (isSessionCheckpointContent(obs.content.body)) {
            ...
            const extracted = extractDeterministic(bodyText, obs.scope, subjectName, obs.source.observed_at);
        ...
        if (!isSessionCheckpointContent(obs.content.body) && useLLM && config.llm.provider !== 'none') {
            if (obs.policy.sensitive) {
                ...
                llmAttempted++;
                try {
                    const llmResult = await extractClaimsLLM(bodyText, obs.scope, obs.source.observed_at, store.getAllEntities(obs.scope), config);
        ...
        for (const claim of allClaims) {
            ...
            const fp = computeStructuredClaimFingerprint(claim.subject_name, claim.predicate, claim.object, obs.scope, claimType);
            ...
            if (existingByFp) {
                if (existingByFp.epistemic_owner === 'user')
                    continue;
                ...
                if (!existingByFp.derived_from.includes(obs.id)) {
                    ...
                    if (commitCtx)
                        commitReflectClaim(dataDir, extended, commitCtx, commitHooks);
                    else
                        appendClaimVersion(dataDir, extended);
            ...
            if (commitCtx)
                commitReflectClaim(dataDir, record, commitCtx, commitHooks);
            else
                appendClaimVersion(dataDir, record);
            claimVersionsWritten++;
        ...
        markComplete(obs.id, obs.scope, allClaims.length === 0 ? 'no_claims' : 'claims_processed', {
...
function findByFingerprint(dataDir, fp) {
...
function findSemanticMatch(dataDir, store, structuredFingerprint) {
    for (const claim of store.getActiveClaims()) {
        const candidate = computeStructuredClaimFingerprint(claim.subject_name, claim.predicate, claim.object, claim.scope, (claim.claim_type ?? 'hypothesis'));
        if (candidate !== structuredFingerprint)
            continue;
