...
function groupBySubject(claims) {
 ...
 */
export function buildAuthorizedClaimSnapshot(policy, store, config) {
    ...
    const entities = new Map(store.getAllEntities(policy.scope).map(entity => [entity.id, entity]));
    ...
    const claims = allClaims.filter(claim => {
        ...
        if (forgotten && !policy.includeForgotten)
            return false;
        if (claim.status === 'superseded' && !policy.includeSuperseded)
            return false;
        if (claim.status === 'stale' && !policy.includeStale)
            return false;
        if (!policy.includeSuperseded && db && !isEffectiveCurrent(claim.id, db))
            return false;
        if (claim.sensitive && (!policy.includeSensitive || !owner))
            return false;
        if (policy.minConfidence !== undefined && claim.confidence < policy.minConfidence)
            return false;
        if (epistemic.size > 0 && !epistemic.has(claim.epistemic))
            return false;
        if (epistemicTags.size > 0
            && !epistemicTags.has(epistemicToTag(claim.epistemic, claim.status)))
            return false;
        if (policy.entityType && entities.get(claim.subject_id)?.type !== policy.entityType)
            return false;
        return true;
    });
    return {
...
//# sourceMappingURL=authorized-claims.js.map