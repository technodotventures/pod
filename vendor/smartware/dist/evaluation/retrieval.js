const RETRIEVAL_QUERY_CATEGORIES = new Set([
    'exact',
    'paraphrase',
    'entity',
    'current',
...
function validateCases(cases) {
    if (cases.length === 0)
        throw new Error('Retrieval arena must contain at least one case');
    ...
    for (const testCase of cases) {
        ...
        if (!testCase.query.trim())
            throw new Error(`Retrieval case ${testCase.id} has an empty query`);
...
function validateObservations(cases, observations) {
    ...
    for (const observation of observations) {
        ...
        if (observation.channel !== observation.channel.trim()) {
            throw new Error(`Retrieval observation channel must not contain surrounding whitespace: ${observation.channel}`);
        }
        const key = `${observation.channel}\u0000${observation.query_id}`;
        if (keys.has(key)) {
            throw new Error(`Duplicate retrieval observation for ${observation.channel}/${observation.query_id}`);
        }
        keys.add(key);
        assertUnique(observation.result_ids, `${observation.channel}/${observation.query_id}.result_ids`);
...
function validateGates(gates) {
    ...
    for (const gate of gates) {
        ...
        assertFiniteNonNegative(gate.max_missing_observations, `${gate.channel}.max_missing_observations`);
...
function validateActivationPolicy(policy) {
    ...
    assertFiniteNonNegative(policy.max_candidate_latency_p95_ms, 'max_candidate_latency_p95_ms');
...
function validateActivationEvidence(evidence) {
    ...
    if (!['synthetic_non_user_data', 'consented_user_data']
        ...
        throw new Error(`Unsupported retrieval activation data classification: ${evidence.data_classification}`);
    }
...
function recallAt(resultIds, expectedIds, limit) {
...
function ndcgAt10(resultIds, expectedIds) {
    if (expectedIds.length === 0)
        return 0;
    const expected = new Set(expectedIds);
    const dcg = resultIds.slice(0, 10).reduce((sum, id, index) => sum + (expected.has(id) ? 1 / Math.log2(index + 2) : 0), 0);
    const idealCount = Math.min(expectedIds.length, 10);
    const ideal = Array.from({ length: idealCount }, (_value, index) => index).reduce((sum, index) => sum + 1 / Math.log2(index + 2), 0);
...
function evaluateQuery(testCase, channel, observation) {
...
function summarize(evaluations) {
    ...
    return {
        ...
        forbidden_hits: evaluations.reduce((sum, result) => sum + result.forbidden_hits.length, 0),
        obsolete_hits: evaluations.reduce((sum, result) => sum + result.obsolete_hits.length, 0),
        missing_observations: evaluations.filter(result => result.missing_observation).length,
        latency_p50_ms: percentile(evaluations.flatMap(result => result.latency_ms === null ? [] : [result.latency_ms]), 0.5),
        latency_p95_ms: percentile(evaluations.flatMap(result => result.latency_ms === null ? [] : [result.latency_ms]), 0.95),
...
function summarizeChannel(channel, evaluations) {
    const categories = {};
    for (const category of new Set(evaluations.map(result => result.category))) {
        categories[category] = summarize(evaluations.filter(result => result.category === category));
    }
    return {
        channel,
...
export function evaluateRetrievalArena(cases, observations, gates = []) {
    validateCases(cases);
    ...
    const summaries = channels.map(channel => summarizeChannel(channel, cases.map(testCase => evaluateQuery(testCase, channel, byKey.get(`${channel}\u0000${testCase.id}`)))));
    const gateResults = gates.map(gate => evaluateGate(summaries.find(summary => summary.channel === gate.channel), gate));
    return {
...
function compareQuery(baseline, candidate) {
    ...
    const regressed = candidateMetrics.some((value, index) => value < baselineMetrics[index] - ACTIVATION_EPSILON);
    if (regressed)
        return -1;
    const improved = candidateMetrics.some((value, index) => value > baselineMetrics[index] + ACTIVATION_EPSILON);
    return improved ? 1 : 0;
}
export function evaluateRetrievalActivation(cases, observations, evidence, policy) {
    validateActivationEvidence(evidence);
    validateActivationPolicy(policy);
    const arena = evaluateRetrievalArena(cases, observations);
    const baseline = arena.channels.find(channel => channel.channel === policy.baseline_channel);
    const candidate = arena.channels.find(channel => channel.channel === policy.candidate_channel);
    if (!baseline) {
    ...
    const baselineByQuery = new Map(baseline.queries_detail.map(result => [result.query_id, result]));
    const winningQueryIds = [];
    const losingQueryIds = [];
    for (const candidateQuery of candidate.queries_detail) {
        ...
        if (comparison > 0)
            winningQueryIds.push(candidateQuery.query_id);
        if (comparison < 0)
            losingQueryIds.push(candidateQuery.query_id);
    }
    const comparison = {
        ...
        deltas: {
            ...
            mean_reciprocal_rank: candidate.mean_reciprocal_rank - baseline.mean_reciprocal_rank,
    ...
    if (evidence.dataset_status !== 'held_out') {
        failures.push(`dataset_status ${evidence.dataset_status} is not held_out`);
    }
    ...
    if (candidate.scored_queries < minScoredQueries) {
        failures.push(`scored_queries ${candidate.scored_queries} is below ${minScoredQueries}`);
    }
    if (baseline.missing_observations > 0) {
        failures.push(`baseline missing_observations ${baseline.missing_observations} exceeds 0`);
    }
    const hardCandidateMaximums = [
    ...
    for (const [actual, metric] of hardCandidateMaximums) {
        if (actual > 0)
            failures.push(`candidate ${metric} ${actual} exceeds 0`);
    }
    const minimumDeltas = [
    ...
    if (policy.max_candidate_latency_p95_ms !== undefined
        ...
        failures.push(`candidate latency_p95_ms ${candidate.latency_p95_ms ?? 'unreported'}`
    ...
    if (policy.max_candidate_cost_usd !== undefined
        ...
        failures.push(`candidate cost_usd ${candidate.cost_usd} exceeds ${policy.max_candidate_cost_usd}`);
    }
    return {
        passed: failures.length === 0,
