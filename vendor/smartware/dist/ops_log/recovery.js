...
import { readOperationIntentRecords, removeOperationIntent, } from './intent.js';
...
function markdownFiles(root) {
    ...
    for (const entry of readdirSync(root, { withFileTypes: true })) {
        ...
        if (entry.isDirectory())
            files.push(...markdownFiles(candidate));
        else if (entry.isFile() && entry.name.endsWith('.md'))
            files.push(candidate);
    }
    return files;
}
function pageOperationId(filePath) {
...
function groupPagesByOperation(wikiDir) {
...
function groupL0ByOperation(observations) {
    const grouped = new Map();
...
function groupL1ByOperation(versions) {
    const grouped = new Map();
    for (const version of versions) {
        if (!version.operation_id)
            continue;
...
function isExactObservationIntent(intent, observation, allObservations) {
...
function isExactReviseIntent(intent, version) {
...
function isExactForgetAudit(intent, observation, allObservations) {
...
function isExactForgetClaim(intent, version) {
...
function isExactReviveIntent(intent, version) {
    return version.state === 'active'
        && version.claim_id === intent.expected.claim_id
        && version.version === intent.expected.version
        && version.operation_id === intent.operation_id
...
function isExactEndorseClaims(intent, versions) {
    if (versions.length !== intent.expected.claims.length)
        return false;
...
export function runRecovery(ctx) {
    ...
    for (const record of readOperationIntentRecords(ctx.opsDir)) {
        if (!record.intent) {
            ...
            if (record.operation_id.startsWith('op_'))
                manualReview.add(record.operation_id);
            continue;
        }
        ...
        if (intent.op === 'forget') {
            ...
            if (existing.length > 0) {
                const matchingCommit = existing.some(entry => entry.op === 'forget'
        ...
        if (intent.op === 'revive') {
            ...
            if (existing.length > 0) {
                const matchingCommit = existing.some(entry => entry.op === 'revive'
                    && entry.actor_id === intent.actor_id
                    && entry.details?.['payload_hash'] === intent.payload_hash
                ...
                if (matchingCommit && exactArtifact)
                    removeOperationIntent(ctx.opsDir, intent.operation_id);
                else
                    manualReview.add(intent.operation_id);
                continue;
        ...
        if (intent.op === 'endorse') {
            ...
            if (existing.length > 0) {
                const matchingCommit = existing.some(entry => entry.op === 'endorse'
                    && entry.actor_id === intent.actor_id
                    && entry.details?.['payload_hash'] === intent.payload_hash
            ...
            const claimsInvalid = claimsPresent && !isExactEndorseClaims({
                ...
                expected: {
                    ...intent.expected,
                    claims: intent.expected.claims.filter(expected => claimArtifacts.some(version => version.claim_id === expected.claim_id && version.version === expected.version)),
                },
            }, claimArtifacts);
        ...
        if (intent.op === 'reflect.auto') {
            ...
            if (existing.length > 0) {
                const matchingCommit = existing.some(entry => entry.op === 'reflect.auto'
                    && entry.actor_id === intent.actor_id
                    && entry.details?.['payload_hash'] === intent.payload_hash
                    && entry.details?.['claim_id'] === intent.expected.claim_id
                    && entry.details?.['record_hash'] === intent.expected.record_hash);
        ...
        if (existing.length > 0) {
            const matchingCommit = intent.op === 'observe'
                ? existing.some(entry => entry.op === intent.op
                    && entry.actor_id === intent.actor_id
                    && entry.details?.['payload_hash'] === intent.payload_hash
            ...
            const matchingArtifact = intent.op === 'observe'
                ? true
                : (l1ByOperation.get(intent.operation_id) ?? []).filter(version => isExactReviseIntent(intent, version)).length === 1;
            if (matchingCommit && matchingArtifact)
                removeOperationIntent(ctx.opsDir, intent.operation_id);
            else
                manualReview.add(intent.operation_id);
            continue;
        ...
        const exact = intent.op === 'observe'
            ? isExactObservationIntent(intent, artifacts[0], observations)
            : isExactReviseIntent(intent, artifacts[0]);
    ...
    if (ctx.wikiDir) {
        for (const filePath of markdownFiles(ctx.wikiDir)) {
            const operationId = pageOperationId(filePath);
            if (!operationId || committed.has(operationId))
                continue;
            orphans.push({
 ...
 */
export function classifyOrphan(_orphan) {
    return 'quarantine';
}
