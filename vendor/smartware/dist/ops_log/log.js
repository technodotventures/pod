 ...
 */
export function appendOpLogEntry(opsDir, entry) {
    if (!OPERATION_ID_PATTERN.test(entry.operation_id)) {
        throw new Error(`Operations log entry rejected: operation_id '${entry.operation_id}' does not match ${OPERATION_ID_PATTERN}`);
    }
...
export function readOpLogDay(opsDir, date) {
...
export function* readAllOpLogEntries(opsDir) {
...
export function loadCommittedOperationIds(opsDir) {
    const ids = new Set();
